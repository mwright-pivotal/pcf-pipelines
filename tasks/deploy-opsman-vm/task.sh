...
EOF
  ...
  old_opsman_path="$(govc find "${GOVC_DATACENTER}" -type m -guest.ipAddress "${OPSMAN_IP}" -runtime.powerState poweredOn)"
  ...
  set +e
  while ! $started; do
      OUTPUT=$(govc vm.info -vm.ipath="${OPSMAN_VM_FOLDER}/${opsman_name}" 2>&1)
